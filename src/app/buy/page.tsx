...
import {
  ...
  TrendingDown, Users, CheckCircle2, ArrowRight,
} from 'lucide-react';
    ...
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-12 xl:gap-20 items-center">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3 mb-10">
                <Link href="#products" className="bg-green-600 hover:bg-green-500 text-white font-bold px-6 py-3 rounded-xl flex items-center gap-2 transition-colors hover:shadow-lg hover:shadow-green-500/20">
                  <Flame className="w-4 h-4" /> Order Briquettes
                </Link>
                <Link href="/buy/orders" className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-semibold px-6 py-3 rounded-xl transition-colors">
                  Track My Order
                </Link>
              </div>
            ...
            {/* Right preview card — real products */}
            {products.length > 0 && (
              <div className="hidden lg:block">
                <div className="bg-slate-900 border border-slate-700/60 rounded-2xl overflow-hidden shadow-2xl shadow-black/40">
                  ...
                  <div className="divide-y divide-slate-800">
                    {products.map((p, i) => {
                      const gradients = ['from-green-600 to-green-800', 'from-emerald-600 to-emerald-800', 'from-teal-600 to-teal-800'];
                      return (
                        <div key={p.id} className="flex items-center gap-4 px-5 py-4">
                          <div className={`bg-gradient-to-br ${gradients[i % 3]} rounded-xl flex items-center justify-center shrink-0 h-11 w-14`}>
                          ...
                          <div className="text-right shrink-0">
                            <p className="text-white font-extrabold">KES {p.unitPrice.toLocaleString()}</p>
                            <p className="text-slate-500 text-xs">{p.weightKg} kg</p>
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div className="px-5 pb-5 pt-3">
                    <Link href="#products" className="flex items-center justify-between w-full bg-green-600 hover:bg-green-500 text-white font-bold py-2.5 px-4 rounded-xl text-sm transition-colors">
                      <span>Browse & Order</span><ArrowRight className="w-4 h-4" />
                    </Link>
                  </div>
                </div>
              </div>
      ...
      <section id="products" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="mb-8">
          ...
          <p className="text-slate-400 text-sm mt-1">Free delivery anywhere in Kirinyaga County · M-Pesa payment</p>
        </div>

        {products.length === 0 ? (
          <p className="text-slate-500 text-center py-12">Products are currently unavailable. Please check back soon.</p>
        ) : (
            ...
            <div>
              ...
              <div className="bg-gradient-to-br from-green-800 via-green-900 to-slate-900 rounded-2xl border border-green-700/30 p-5 relative overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <p className="text-green-200/70 text-sm mb-4">Join 47 schools already saving money and protecting forests.</p>
                  <Link href="#products" className="inline-flex items-center gap-2 bg-white hover:bg-green-50 text-green-800 font-bold px-5 py-2.5 rounded-xl text-sm transition-colors">
                    <Flame className="w-4 h-4" /> Order Now <ArrowRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              </div>
            </div>
