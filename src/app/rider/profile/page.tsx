...
import { MapPin, Phone, Bike, KeyRound, LogOut, HelpCircle, Download, ChevronRight, Loader2 } from 'lucide-react';
import type { ElementType } from 'react';
import { useRiderProfileQuery, useRiderEarningsQuery } from '@/features/rider/hooks/useRider';
...
export default function RiderProfilePage() {
  const { data: profile, isLoading } = useRiderProfileQuery();
  const { data: earnings } = useRiderEarningsQuery();
  const { logout, user } = useAuthStore();
  const router = useRouter();

  const displayName = profile?.fullName ?? user?.displayName ?? 'Rider';
      ...
      <div className="flex flex-col items-center text-center mb-8">
        ...
        {profile && (
          <div className="flex items-center gap-1.5 mt-2.5 text-green-400 text-xs bg-green-400/10 border border-green-400/20 px-3 py-1 rounded-full">
            <span className={`w-1.5 h-1.5 rounded-full ${profile.active ? 'bg-green-400 animate-pulse' : 'bg-slate-500'}`} />
            {profile.active ? 'Active' : 'Inactive'} Rider · {profile.ward} Ward
          </div>
        )}
      </div>
      ...
      {profile && (
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-4 mb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { icon: MapPin, label: 'Ward',    value: profile.ward },
              { icon: Bike,   label: 'Status',  value: profile.active ? 'Active' : 'Inactive' },
      ...
      {earnings && (
        <div className="bg-slate-900 rounded-2xl border border-slate-800 p-4 mb-4">
          ...
          <div className="flex gap-3">
            {[
              { label: 'Pickups',  value: String(earnings.pickupCount) },
              { label: 'Kg done',  value: `${earnings.totalWeightKg.toLocaleString()} kg` },
              { label: 'Earned',   value: formatKES(earnings.total) },
            ].map(({ label, value }) => (
      ...
      {profile && (
        <div className="bg-slate-900 rounded-2xl border border-slate-800 px-4 py-3 mb-4 flex items-center justify-between">
          ...
          <span className="text-sky-400 text-xs font-mono tracking-widest">
            {profile.id.slice(-12).toUpperCase()}
          </span>
        </div>
      )}
